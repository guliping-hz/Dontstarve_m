...
function Health:SetVal(val, cause)
    ...
    if self.currenthealth <= 0 then
        -- 如果是玩家则回血
        -- if self.inst.HasTag("player") and GLOBAL.baba then
        ...
        --     GLOBAL.deathCnt + = 1
        --     print("you have dead for", GLOBAL.deathCnt, "times")
        -- end
        if self.inst:HasTag("player") and NoDie then
            self.currenthealth = 100

            self.deathCnt = self.deathCnt+1
            print("you have dead for", self.deathCnt, "times")
        end
    end
