...
function PlayerActionPicker:GetRightClickActions( target_ent, position )
    ...
    -- We always want to toggle the flashlight on RMB, so get rid of all this logic and just check for inv actions (above)
...
function PlayerActionPicker:DoGetMouseActions( force_target )
    ...
    if force_target or not hud then
    
        local target = force_target
		...
		if not target then        
			...
			for k,v in pairs(ents) do
				if not ignore_player or not v:HasTag("player") and v:HasTag("CLICK") and v.Transform then
					target = v
					break
				end
			end
		...
		if (target and target:IsValid() and target.Transform and (target:HasTag("player") or TheSim:GetLightAtPoint(target.Transform:GetWorldPosition()) >= 0)) or TheSim:GetLightAtPoint(position.x,position.y,position.z) >= 0 then    
