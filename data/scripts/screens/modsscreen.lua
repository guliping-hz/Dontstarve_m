...
local mid_col = RESOLUTION_X * 0.07
local left_col = -RESOLUTION_X * 0.3
local right_col = RESOLUTION_X * 0.37
...
local ModsScreen = Class(Screen, function(self, cb)
    ...
    self.modconfigbutton:SetScale(0.85)
...
function ModsScreen:OnStatsQueried(result, isSuccessful, resultCode)
    ...
    for i = 1, #self.modlinks do
        if jsonresult["modnames"] ~= nil then
    ...
    if jsonresult["modfeature"] ~= nil then
...
function ModsScreen:CreateDetailPanel()
    ...
    if #self.modnames > 0 then
        ...
        self.detailauthor:SetColour(1, 1, 1, 1)
        ...
        self.detailcompatibility:SetColour(1, 1, 1, 1)
...
function ModsScreen:RefreshOptions()
    ...
    for k = 1, page_total do
        ...
        elseif modStatus == "WILL_DISABLE" then
            opt.status:SetColour(0.7, 0.7, 0.7, 1)
        ...
        opt.RoGcompatible:SetScale(0.35, 0.33, 1)
        ...
        opt.DScompatible:SetScale(0.35, 0.33, 1)
        ...
        if KnownModIndex:IsModEnabled(modname) then
            ...
            opt.image:SetTint(1, 0.5, 0.5, 1)
            opt.checkbox:SetTexture("images/ui.xml", "button_checkbox1.tex")
            opt.checkbox:SetTint(1, 0.5, 0.5, 1)
            opt.name:SetColour(0.7, 0.7, 0.7, 1)
            opt.RoGcompatible:SetTint(1, 0.5, 0.5, 1)
            opt.DScompatible:SetTint(1, 0.5, 0.5, 1)
        end
        ...
        opt:SetPosition(0, (display_rows - 1) * spacing * 0.5 - (k - 1) * spacing - 0, 0)
